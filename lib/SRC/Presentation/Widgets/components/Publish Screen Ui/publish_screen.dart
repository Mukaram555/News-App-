...
class _PublishScreenState extends State<PublishScreen> {
  ...
  //       user = response.user;
  //       if (user == null) {
  //         // print('Sign in failed: ${response.error?.message}');
  //         print('Signed in! User ID: user');
  //         return null;
  //       }
  //       print('Signed in! User ID: ${user.id}');
  //     } else {
  //       print('User already signed in: ${user.id}');
  //     }
  //
  //     final fileName =
  //         'news_images/${DateTime.now().millisecondsSinceEpoch}.jpg';
  //
  //     log("The File Path is ${imageFile.path.toLowerCase()}");
  //
  //     // Upload the file
  //     final imageNewUrl = await client.storage
  //         .from('uploads')
  //         .upload(fileName, imageFile);
  //
  //     // Manual URL construction (this will work)
  //     return imageNewUrl;
  //   } catch (e) {
  ...
  //     print("The User Id we have ${user?.id}");
  //     final supabase = Supabase.instance.client;
  //     final fileName =
  //         'news_images/${DateTime.now().millisecondsSinceEpoch}.jpg';
  //
  //     log("Teh Fiele Pth is ${imageFile.path.toLowerCase()}");
  //
  //     // Upload the file
  //     final imageNewUrl = await supabase.storage
  //         .from('uploads')
  //         .upload(fileName, imageFile);
  //
  //     // Manual URL construction (this will work)
                                      ...
                                      children: [
                                        IconButton(
                                          onPressed: () async {
                                            ...
                                            if (kDebugMode) {
                                              print(
                                                '========== photos ${pickerImage.value?.path} pickerImage.value',
                                              );
                                            }
            ...
            children: [
              ...
              InkWell(
                onTap: () async {
                  ...
                  // final file = File(pickerImage.value!.path);
                  // if (!await file.exists()) {
                  //   print(
                  //     'File does not exist at path: ${pickerImage.value!.path}',
                  //   );
                  //   return;
                  // }
                  //
                  ...
                  //
                  // // Upload to Supabase
                  // print('Final Image URL: ${uploadImageToSupabase(file)}');
                            ...
                            actions: [
                              GestureDetector(
                                onTap: () {
                                  ...
                                  Navigator.pop(context); // now close screen
                                },
                                child: Container(
                                  height: 30.h,
                                  width: 60.w,
